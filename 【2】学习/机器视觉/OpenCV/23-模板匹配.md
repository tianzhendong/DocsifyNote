...
![](https://gitee.com/tianzhendong/img/raw/master//images/cv2_understand_template_matching.jpg)

学习使用模板匹配在**图像中寻找物体**。图片等可到文末引用处下载。
...
匹配函数返回的是**一副灰度图，最白的地方表示最大的匹配**。使用`cv2.minMaxLoc()`函数可以得到最大匹配值的坐标，以这个点为左上角角点，模板的宽和高画矩形就是匹配的位置了：
...
![](https://gitee.com/tianzhendong/img/raw/master//images/cv2_ccoeff_matching_template.jpg)
...
![](https://gitee.com/tianzhendong/img/raw/master//images/cv2_np_where_function.jpg)
